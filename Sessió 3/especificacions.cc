...
void Cjt_estudiants::afegir_estudiant(const Estudiant& est, bool& b) {
  ...
  b = false;
  while (i >= 0 and not trobat and not b) {
    if ( dni> vest[i].consultar_DNI()) trobat = true; // dni_entrat > dni_consultat
...
void Cjt_estudiants::consultar_estudiant(int dni, bool& b) const {
  int i=cerca_dicot(vest,0,nest-1,dni);
  b = false;
  if (i != -1) {
    return vest[i];
