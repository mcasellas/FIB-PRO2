...
{
    int i = 0;
    while (dni != vest[i].consultar_DNI()) ++i; //trobem la posició de l'estudiant

    for (int j = i; j < nest - 1; ++j) vest[j] = vest[j + 1]; //movem els elements superiors a la posició d'abans
    --nest; //reduim el nombre total d'estudiants
    if (i == imax) recalcular_posicio_imax(); //si era nota màxima la tornem a calcular
    else if (imax > i) --imax; //reduim sinò l'element max
}
...
void Cjt_estudiants::recalcular_posicio_imax(){
    ...
    imax = -1; //establim que no hi ha posició de nota màxima
    for(int i = 0; i < nest; ++i){
        if(imax >= 0){

            if (vest[i].te_nota()) {
                if (vest[i].consultar_nota() > vest[imax].consultar_nota()) imax = i; //si la nota de la consulta és major que l'actual imax la canviem
                else if (vest[i].consultar_nota() == vest[imax].consultar_nota() and vest[i].consultar_DNI() < vest[imax].consultar_DNI()) imax = i; // Si té la mateixa nota màxima pero DNi més petitm, canviem.
            }


        } else if(vest[i].te_nota()){ //agafem el primer element amb nota com a primer imax
            imax = i;
        }
    }
}
